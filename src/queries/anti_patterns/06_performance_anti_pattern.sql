...
WITH filtered_employees AS (
    SELECT * FROM employees WHERE salary > 50000 AND LENGTH(name) > 5 AND department IN ('Engineering', 'Marketing', 'Sales')
)
SELECT 
    e1.id as emp1_id,
...
FROM 
    filtered_employees e1
JOIN 
    filtered_employees e2 ON e1.id != e2.id
JOIN 
    filtered_employees e3 ON e1.id != e3.id AND e2.id != e3.id
