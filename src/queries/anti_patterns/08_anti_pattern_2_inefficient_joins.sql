...
FROM 
    (SELECT * FROM employees WHERE salary > 60000) e
JOIN /*+ BROADCAST(d) */ (SELECT dept_name, category, manager FROM departments WHERE category IN ('Tech', 'Business')) d ON e.department = d.dept_name
JOIN (SELECT * FROM projects WHERE budget > 50000 AND start_date >= '2024-01-01' AND end_date <= '2024-12-31') p ON e.department = p.department
JOIN (SELECT * FROM large_dataset WHERE amount > 10000) l ON e.id = l.employee_id
